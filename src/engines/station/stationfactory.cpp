 ...
 */
QFuture<bool> StationEngine::Factory::insertStationWithoutFacilitiesIntoDatabase(const QStringList &station)
{
    QSqlQuery query(this->db()->database());
    query.prepare("INSERT INTO stations ("
                  "uri, "
 ...
 * @package StationEngine
 * @private
 * Adds a station to the memory cache to avoid duplicate StationEngine::Station objects.
 ...
 */
Database::Manager *StationEngine::Factory::db() const
{
    return m_db;
}
 ...
 */
void StationEngine::Factory::setDb(Database::Manager *db)
{
    m_db = db;
}
