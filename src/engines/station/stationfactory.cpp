    ...
    while (query.next()) {
      ...
      if (hasFacilities) {
        ...
        station = new StationEngine::Station(
            uri, name, country, position, address, hasTicketVendingMachine,
            hasLuggageLockers, hasFreeParking, hasTaxi, hasBicycleSpots,
            hasBlueBike, hasBus, hasTram, hasMetro, hasWheelchairAvailable,
            hasRamp, disabledParkingSpots, hasElevatedPlatform, hasEscalatorUp,
            hasEscalatorDown, hasElevatorPlatform, hasHearingAidSignal,
            openingHours, averageStopTimes, platforms);
      } else {
        station = new StationEngine::Station(uri, name, country, position,
                                             averageStopTimes, platforms);
      }
 ...
 */
QFuture<bool>
StationEngine::Factory::insertStationWithoutFacilitiesIntoDatabase(
    const QStringList &station) {
  QSqlQuery query(this->db()->database());
  query.prepare("INSERT INTO stations ("
                "uri, "
                "name, "
 ...
 */
QFuture<bool> StationEngine::Factory::insertPlatformIntoDatabase(
    const QStringList &platform) {
  QSqlQuery query(this->db()->database());
 ...
 * routing calculation.
 */
void StationEngine::Factory::addStationToCache(
    StationEngine::Station *station) {
 ...
 */
QMap<QUrl, QString>
StationEngine::Factory::getPlatformsByStationURI(const QUrl &uri) {
  ...
  while (query.next()) {
    platformsMap.insert(query.value(0).toUrl(),   // Platform URI
