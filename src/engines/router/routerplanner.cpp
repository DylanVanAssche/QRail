...
QRail::RouterEngine::Planner *QRail::RouterEngine::Planner::m_instance =
    nullptr;
 ...
 */
void QRail::RouterEngine::Planner::getConnections(
    const QUrl &departureStation, const QUrl &arrivalStation,
    const QDateTime &departureTime, const qint16 &maxTransfers) {
  /*
...
void QRail::RouterEngine::Planner::parsePage(QRail::Fragments::Page *page) {
  QMutexLocker locker(&syncThreadMutex); // Lock processing to enforce the
                                         // DESCENDING order of departure times
  bool hasPassedDepartureTimeLimit =
      false; // Flag to check if we're passed the departureTime
  QDateTime INFINITE_TIME = QDateTime(QDateTime::currentDateTime().addYears(
      1)); // Fake an infinite time by adding 1 year to the current date, this
           // is required for the Profile Scan Algortihm to work
  qint16 INFINITE_TRANSFERS = 32767; // Fake an infinite number of transfers by
                                     // setting it to 32767 (16 bits signed
                                     // 01111111 11111111)
  ...
  for (qint16 fragIndex = page->fragments().size() - 1; fragIndex >= 0;
       fragIndex--) {
    ...
    QDateTime T1_walkingArrivalTime, T2_stayOnTripArrivalTime,
        T3_transferArrivalTime, Tmin_earliestArrivalTime;
    qint16 T1_transfers, T2_transfers, T3_transfers, Tmin_transfers;
    QRail::Fragments::Fragment *newExitTrainFragment; // Save the connection
                                                      // when we exit the train
                                                      // for a transfer

    // Calculate T1, the time when walking from the current stop to the
    // destination
    if (fragment->arrivalStationURI() == this->arrivalStationURI()) {
      ...
      T1_walkingArrivalTime =
          fragment->arrivalTime(); // Due the footpath limitation, we arrive at
                                   // our destination when we arrive with the
                                   // vehicle.
      T1_transfers =
          0; // Walking, no transfers between arrival and destination.
    } else {
      /*
    ...
    if (this->TArray().contains(fragment->tripURI())) {
       ...
       */
      T2_stayOnTripArrivalTime =
          this->TArray().value(fragment->tripURI())->arrivalTime();
    ...
    // Calculate T3, the time of arrival when taking the best possible transfer
    // in this station.
    if (this->SArray().contains(fragment->arrivalStationURI())) {
       ...
       */
      qint16 position =
          this->SArray().value(fragment->arrivalStationURI()).size() - 1;
      QRail::RouterEngine::StationStopProfile *stopProfile =
          this->SArray().value(fragment->arrivalStationURI()).at(position);

      while ((((stopProfile->departureTime().toMSecsSinceEpoch() -
                INTRA_STOP_FOOTPATH_TIME * MILISECONDS_TO_SECONDS_MULTIPLIER) <
               fragment->arrivalTime().toMSecsSinceEpoch()) ||
      ...
      if (((stopProfile->departureTime().toMSecsSinceEpoch() -
            INTRA_STOP_FOOTPATH_TIME * MILISECONDS_TO_SECONDS_MULTIPLIER) >=
           fragment->arrivalTime().toMSecsSinceEpoch()) &&
          (stopProfile->transfers() <= this->maxTransfers())) {
        /*
        ...
        T3_transferArrivalTime =
            stopProfile->arrivalTime().addSecs(TRANSFER_EQUIVALENT_TRAVEL_TIME);
        T3_transfers = stopProfile->transfers() +
                       1; // We transfer here, increment the number of transfers
      } else {
        /*
     ...
     * (updating the arrays).
     */
    if (Tmin_earliestArrivalTime == INFINITE_TIME ||
        newExitTrainFragment == nullptr) {
      continue;
    }
    ...
    if (this->TArray().contains(fragment->tripURI())) {
      if ((Tmin_earliestArrivalTime ==
           this->TArray().value(fragment->tripURI())->arrivalTime()) &&
          (this->TArray()
               .value(fragment->tripURI())
               ->arrivalConnection()
               ->arrivalStationURI() != this->arrivalStationURI()) &&
          T3_transferArrivalTime == T2_stayOnTripArrivalTime &&
          this->SArray().contains(this->TArray()
                                      .value(fragment->tripURI())
                                      ->arrivalConnection()
                                      ->arrivalStationURI()) &&
        ...
        QRail::RouterEngine::StationStopProfile *currentStationStopProfile =
            new QRail::RouterEngine::StationStopProfile(
                fragment->departureTime(), Tmin_earliestArrivalTime, fragment,
                currentExitTrainFragment, Tmin_transfers);
        ...
        if (currentFirstReachableProfile) {
          currentTransferDuration =
              currentExitTrainFragment->arrivalTime().secsTo(
                  currentFirstReachableProfile->departureTime());
        } else {
          qCritical() << "Unable to get first reachable connection!";
          emit this->error(
              "Unable to retrieve the first reachable connection!");
        }
        ...
        QRail::RouterEngine::StationStopProfile *newStationStopProfile =
            new QRail::RouterEngine::StationStopProfile(
                fragment->departureTime(), Tmin_earliestArrivalTime, fragment,
                newExitTrainFragment, Tmin_transfers);
        ...
        if (currentTransferDuration > 0 && newTransferDuration > 0 &&
          ...
          QRail::RouterEngine::TrainProfile *newTrainProfile =
              new QRail::RouterEngine::TrainProfile(Tmin_earliestArrivalTime,
                                                    newExitTrainFragment,
                                                    Tmin_transfers);
          QMap<QUrl, QRail::RouterEngine::TrainProfile *> T = this->TArray();
          T.insert(fragment->tripURI(), newTrainProfile); // Key is
                                                          // automatically
                                                          // updated when key
                                                          // already exists
          this->setTArray(T);
      ...
      if (Tmin_earliestArrivalTime <
        ...
        QRail::RouterEngine::TrainProfile *fasterTrainProfile =
            new QRail::RouterEngine::TrainProfile(
                Tmin_earliestArrivalTime, newExitTrainFragment, Tmin_transfers);
        QMap<QUrl, QRail::RouterEngine::TrainProfile *> T = this->TArray();
        T.insert(fragment->tripURI(), fasterTrainProfile); // Key is
                                                           // automatically
                                                           // updated when key
                                                           // already exists
        this->setTArray(T);
      ...
      T.insert(fragment->tripURI(),
               nonExistingTrainProfile); // Key is automatically updated when
                                         // key already exists
      this->setTArray(T);
    }
    ...
    if (this->SArray().contains(fragment->departureStationURI())) {
      qint16 numberOfPairs =
          this->SArray().value(fragment->departureStationURI()).size();
      QRail::RouterEngine::StationStopProfile *existingStationStopProfile =
          this->SArray()
              .value(fragment->departureStationURI())
              .at(numberOfPairs - 1);

      if (updatedStationStopProfile->arrivalTime() <
          existingStationStopProfile->arrivalTime()) {
        // Replace existing StationStopProfile at the back when departure times
        // are equal
        if (updatedStationStopProfile->departureTime() ==
            existingStationStopProfile->departureTime()) {
          /*
          ...
          QMap<QUrl, QList<QRail::RouterEngine::StationStopProfile *>> S =
              this->SArray();
          QList<QRail::RouterEngine::StationStopProfile *> SProfiles =
              S.value(fragment->departureStationURI());
          SProfiles.replace(numberOfPairs - 1,
                            updatedStationStopProfile); // Replace profile when
                                                        // departure times are
                                                        // equal
        ...
        // StationStopProfile at the back
          ...
          QList<QRail::RouterEngine::StationStopProfile *> SProfiles =
              S.value(fragment->departureStationURI());
          SProfiles.append(updatedStationStopProfile); // Add profile when we
                                                       // have different
                                                       // departure times
          S.insert(fragment->departureStationURI(), SProfiles);
          this->setSArray(S);
  ...
  if (hasPassedDepartureTimeLimit) {
    ...
    if (this->SArray().contains(this->departureStationURI())) {
      ...
      foreach (QRail::RouterEngine::StationStopProfile *profile,
               this->SArray().value(this->departureStationURI())) {
        QList<QRail::RouterEngine::RouteLeg *> legs =
            QList<QRail::RouterEngine::RouteLeg *>();

        // TODO: investigate the use of a do-while loop to avoid code repetition
        while (profile->arrivalConnection()->arrivalStationURI() !=
          ...
          QRail::RouterEngine::RouteLegEnd *departureLeg =
              new QRail::RouterEngine::RouteLegEnd(
                  ...
                  this->stationFactory()->getStationByURI(
                      profile->departureConnection()->departureStationURI()),
                  QString("?"), true,
                  profile->departureConnection()->departureDelay(), false,
                  profile->departureConnection()->departureTime() <
          ...
          QRail::RouterEngine::RouteLegEnd *arrivalLeg =
              new QRail::RouterEngine::RouteLegEnd(
                  ...
                  this->stationFactory()->getStationByURI(
                      profile->arrivalConnection()->arrivalStationURI()),
                  QString("?"), true,
                  profile->arrivalConnection()->departureDelay(), false,
                  profile->arrivalConnection()->arrivalTime() <
                      QDateTime::currentDateTime(),
                  QRail::VehicleEngine::Stop::OccupancyLevel::UNSUPPORTED);
        ...
        qDebug() << profile->arrivalConnection()->arrivalStationURI()
                 << profile->arrivalConnection()->arrivalTime().toString(
                        "hh:mm");
#endif
        ...
        QRail::RouterEngine::Route *route =
            new QRail::RouterEngine::Route(legs);
      ...
      std::sort(routeList.begin(), routeList.end(),
                [](const QRail::RouterEngine::Route *a,
    ...
    // Emit routesFound signal as soon as we parsed a full LC page without
    // results
    emit this->routesFound(this->routes());
    this->setRoutes(QList<QRail::RouterEngine::Route *>()); // Clear the routes
                                                            // list for a new
                                                            // calculation
  }
}
 ...
 */
QRail::RouterEngine::StationStopProfile *
QRail::RouterEngine::Planner::getFirstReachableConnection(
   ...
   */
  QList<QRail::RouterEngine::StationStopProfile *> options =
      this->SArray().value(
  ...
  while (i > 0 && (options.at(i)->arrivalTime().toMSecsSinceEpoch() !=
                   (arrivalProfile->arrivalTime().toMSecsSinceEpoch() -
                    TRANSFER_EQUIVALENT_TRAVEL_TIME *
 ...
 */
QDateTime QRail::RouterEngine::Planner::calculateArrivalTime(
    const QDateTime &departureTime) {
  // The current implementation guesses the arrival time
  // Calculating the arrival time can reduce the number of LC pages we need to
  // fetch
 ...
 */
void QRail::RouterEngine::Planner::setArrivalStationURI(
 ...
 */
void QRail::RouterEngine::Planner::setDepartureStationURI(
...
QRail::RouterEngine::Planner::routes() const {
  return m_routes;
}
 ...
 */
void QRail::RouterEngine::Planner::setRoutes(
...
QRail::RouterEngine::Planner::fragmentsFactory() const {
 ...
 */
void QRail::RouterEngine::Planner::setFragmentsFactory(
    QRail::Fragments::Factory *factory) {
 ...
 * *stationFactory.
 */
void QRail::RouterEngine::Planner::setStationFactory(
...
QRail::RouterEngine::Planner::TArray() const {
  return m_TArray;
}
 ...
 */
void QRail::RouterEngine::Planner::setTArray(
 ...
 */
QMap<QUrl, QList<QRail::RouterEngine::StationStopProfile *>>
QRail::RouterEngine::Planner::SArray() const {
  return m_SArray;
}
 ...
 */
void QRail::RouterEngine::Planner::setSArray(
    const QMap<QUrl, QList<QRail::RouterEngine::StationStopProfile *>>
        &SArray) {
  m_SArray = SArray;
}
