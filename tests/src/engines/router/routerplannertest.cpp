...
void RouterEngine::PlannerTest::initCSAPlannerTest()
{
    // Get a Planner instance
    planner = RouterEngine::Planner::getInstance();

    // Let the Qt meta object system know how it should handle our custom QObjects
    qRegisterMetaType< QList<RouterEngine::Route*> >("QList<Route*>");
...
void RouterEngine::PlannerTest::runCSAPlannerTest()
{
    ...
    QBENCHMARK {
        ...
        QEventLoop loop;
        connect(planner, SIGNAL(routesFound(QList<RouterEngine::Route*>)), &loop, SLOT(quit()));
        loop.exec();
    }
}
...
void RouterEngine::PlannerTest::processRouteFound(const QList<RouterEngine::Route *> &routes)
{
    qDebug() << "CSA found" << routes.size() << "possible routes";
    foreach(RouterEngine::Route *route, routes) {
        ...
        foreach(RouterEngine::Transfer *transfer, route->transfers()) {
            if(transfer->type() == RouterEngine::Transfer::Type::TRANSFER) {
            ...
            else if(transfer->type() == RouterEngine::Transfer::Type::DEPARTURE) {
            ...
            else if(transfer->type() == RouterEngine::Transfer::Type::ARRIVAL) {
