 ...
 */
RouterEngine::Planner::Planner(QObject *parent) : QObject(parent)
{
    // Init RouterEngine::Planner
 ...
 * @author Dylan Van Assche
 * @date 09 Aug 2018
 * @brief Gets a RouterEngine::Planner instance
 * @param QObject *parent = nullptr
 * @return RouterEngine::Planner *planner
 * @package RouterEngine
 * @public
 * Constructs a RouterEngine::Planner if none exists and returns the instance.
 */
RouterEngine::Planner *RouterEngine::Planner::getInstance()
{
    // Singleton pattern
    if(m_instance == nullptr) {
        qDebug() << "Generating new RouterEngine::Planner";
 ...
 */
void RouterEngine::Planner::getConnections(const QUrl &departureStation, const QUrl &arrivalStation, const QDateTime &departureTime, const qint16 &maxTransfers)
{
    /*
    ...
    this->setTArray(QMap<QUrl, RouterEngine::TrainProfile *>());
    this->setSArray(QMap<QUrl, QList<RouterEngine::StationStopProfile *> >());
 ...
 */
void RouterEngine::Planner::parsePage(Fragments::Page *page)
{  
    ...
    for(qint16 fragIndex = page->fragments().size() - 1; fragIndex >= 0; fragIndex--) {
        ...
        if(this->SArray().contains(fragment->arrivalStationURI())) {
            ...
            RouterEngine::StationStopProfile *stopProfile = this->SArray().value(fragment->arrivalStationURI()).at(position);
        ...
        if(this->TArray().contains(fragment->tripURI())) {
            if((Tmin_earliestArrivalTime == this->TArray().value(fragment->tripURI())->arrivalTime())
                ...
                // Current situation
                RouterEngine::StationStopProfile *currentStationStopProfile = new RouterEngine::StationStopProfile(
                ...
                RouterEngine::StationStopProfile *currentFirstReachableProfile = this->getFirstReachableConnection(currentStationStopProfile);
                ...
                // New situation
                RouterEngine::StationStopProfile *newStationStopProfile = new RouterEngine::StationStopProfile(
                            fragment->departureTime(),
                            Tmin_earliestArrivalTime,
                            fragment,
                ...
                RouterEngine::StationStopProfile *newFirstReachableProfile = this->getFirstReachableConnection(newStationStopProfile);
                ...
                if(currentTransferDuration > 0 && newTransferDuration > 0 && newTransferDuration > currentTransferDuration) {
                    RouterEngine::TrainProfile *newTrainProfile = new RouterEngine::TrainProfile(
                                Tmin_earliestArrivalTime,
                                newExitTrainFragment,
                                Tmin_transfers
                                );
                    QMap<QUrl, RouterEngine::TrainProfile *> T = this->TArray();
            ...
            if(Tmin_earliestArrivalTime < this->TArray().value(fragment->tripURI())->arrivalTime()) {
                RouterEngine::TrainProfile *fasterTrainProfile = new RouterEngine::TrainProfile(
                            Tmin_earliestArrivalTime,
                            newExitTrainFragment,
                            Tmin_transfers
                            );
            ...
            RouterEngine::TrainProfile *nonExistingTrainProfile = new RouterEngine::TrainProfile(
                        Tmin_earliestArrivalTime,
                        newExitTrainFragment,
                        Tmin_transfers
                        );
        ...
        RouterEngine::StationStopProfile *updatedStationStopProfile = new RouterEngine::StationStopProfile(
                    fragment->departureTime(),
                    Tmin_earliestArrivalTime,
                    fragment,
        ...
        if(this->SArray().contains(fragment->departureStationURI())) {
            ...
            RouterEngine::StationStopProfile *existingStationStopProfile = this->SArray().value(fragment->departureStationURI()).at(numberOfPairs - 1);
            ...
            if(updatedStationStopProfile->arrivalTime() < existingStationStopProfile->arrivalTime()) {
                ...
                if(updatedStationStopProfile->departureTime() == existingStationStopProfile->departureTime()) {
                     ...
                     * issue.
                     *
                     * OUTPUT: /home/dylan/Projects/harbour-berail/src/linkedconnections/routerplanner.cpp:395: error: passing
                    ...
                    QMap<QUrl, QList<RouterEngine::StationStopProfile *> > S = this->SArray();
                    QList<RouterEngine::StationStopProfile *> SProfiles = S.value(fragment->departureStationURI());
        ...
        // New entry in the S array
            ...
            QMap<QUrl, QList<RouterEngine::StationStopProfile *> > S = this->SArray();
            QList<RouterEngine::StationStopProfile *> stationStopProfileList = QList<RouterEngine::StationStopProfile *>();
        ...
        foreach(QUrl k, this->SArray().keys()) {
            qDebug() << k.toString();
            foreach(RouterEngine::StationStopProfile* p, this->SArray().value(k)) {
    ...
    if(hasPassedDepartureTimeLimit) {
        ...
        if(this->SArray().contains(this->departureStationURI())) {
            ...
            foreach(RouterEngine::StationStopProfile *profile, this->SArray().value(this->departureStationURI())) {
                QList<RouterEngine::RouteLeg *> legs = QList<RouterEngine::RouteLeg *>();
                ...
                while(profile->arrivalConnection()->arrivalStationURI() != this->arrivalStationURI()) {
...
#endif
                    RouterEngine::RouteLegEnd *departureLeg = new RouterEngine::RouteLegEnd(
                    ...
                    RouterEngine::RouteLegEnd *arrivalLeg = new RouterEngine::RouteLegEnd(
                    ...
                    RouterEngine::RouteLeg *routeLeg = new RouterEngine::RouteLeg(
                                RouterEngine::RouteLeg::Type::TRAIN,
                                vehicle,
                                departureLeg,
                                arrivalLeg
                                );
                ...
                RouterEngine::Route *route = new RouterEngine::Route(
                            legs
                            );

                QList<RouterEngine::Route *> routeList = this->routes();
             ...
             */
            QList<RouterEngine::Route *> routeList = this->routes();
            std::sort(routeList.begin(), routeList.end(), [](const RouterEngine::Route* a, const RouterEngine::Route* b) -> bool {
        ...
        this->setRoutes(QList<RouterEngine::Route *>()); // Clear the routes list for a new calculation
    }
}
 ...
 * @param RouterEngine::StationStopProfile *arrivalProfile
 * @return RouterEngine::StationStopProfile *profile
 * @package RouterEngine
 * @private
 * Tries to find the first reachable connection for a given
 * RouterEngine::StationStopProfile *arrivalProfile and returns it.
 */
RouterEngine::StationStopProfile *RouterEngine::Planner::getFirstReachableConnection(RouterEngine::StationStopProfile *arrivalProfile)
{
    /*
     ...
     */
    QList<RouterEngine::StationStopProfile *> options = this->SArray().value(arrivalProfile->arrivalConnection()->arrivalStationURI());
    ...
    foreach(RouterEngine::StationStopProfile* profile, options) {
        qDebug() << "\t" << profile->departureConnection()->departureStationURI().toString() << profile->departureConnection()->departureTime().toString("hh:mm");
    }
#endif
...
void RouterEngine::Planner::processing(Fragments::Page *page)
{
    ...
    QtConcurrent::run(this, &RouterEngine::Planner::parsePage, page);
}
 ...
 */
QDateTime RouterEngine::Planner::calculateArrivalTime(const QDateTime &departureTime)
{
 ...
 */
qint16 RouterEngine::Planner::maxTransfers() const
{
 ...
 */
void RouterEngine::Planner::setMaxTransfers(const qint16 &maxTransfers)
{
 ...
 */
QDateTime RouterEngine::Planner::arrivalTime() const
{
 ...
 */
void RouterEngine::Planner::setArrivalTime(const QDateTime &arrivalTime)
{
 ...
 */
QDateTime RouterEngine::Planner::departureTime() const
{
 ...
 */
void RouterEngine::Planner::setDepartureTime(const QDateTime &departureTime)
{
 ...
 */
void RouterEngine::Planner::setArrivalStationURI(const QUrl &arrivalStationURI)
{
 ...
 */
QUrl RouterEngine::Planner::departureStationURI() const
{
 ...
 */
void RouterEngine::Planner::setDepartureStationURI(const QUrl &departureStationURI)
{
 ...
 * @return const QList<RouterEngine::Route *> routes
 * @package RouterEngine
 * @public
 * Gets the routes retrieved by the Connection Scan Algorithm (CSA) and returns it.
 */
QList<RouterEngine::Route *> RouterEngine::Planner::routes() const
{
    return m_routes;
}
 ...
 * @param const QList<RouterEngine::Route *> &routes
 * @package RouterEngine
 * @private
 * Sets the routes retrieved by the Connection Scan Algorithm (CSA) to the given QList<RouterEngine::Route *> &routes.
 */
void RouterEngine::Planner::setRoutes(const QList<RouterEngine::Route *> &routes)
{
    m_routes = routes;
}
 ...
 */
Fragments::Factory *RouterEngine::Planner::fragmentsFactory() const
{
 ...
 */
void RouterEngine::Planner::setFragmentsFactory(Fragments::Factory *factory)
{
 ...
 */
StationEngine::Factory *RouterEngine::Planner::stationFactory() const
{
 ...
 */
void RouterEngine::Planner::setStationFactory(StationEngine::Factory *stationFactory)
{
 ...
 * @return const QMap<QUrl, QList<RouterEngine::StationStopProfile *> > TArray
 * @package RouterEngine
 * @public
 * Gets the T array for the Connection Scan Algorithm (CSA) and returns it.
 */
QMap<QUrl, RouterEngine::TrainProfile *> RouterEngine::Planner::TArray() const
{
    return m_TArray;
}
 ...
 * @date 09 Aug 2018
 * @brief Sets T array
 * @param const QMap<QUrl, RouterEngine::TrainProfile *> &TArray
 * @package RouterEngine
 * @private
 * Sets the T array for the Connection Scan Algorithm (CSA) to the given QMap<QUrl, RouterEngine::TrainProfile *> &TArray.
 */
void RouterEngine::Planner::setTArray(const QMap<QUrl, RouterEngine::TrainProfile *> &TArray)
{
    m_TArray = TArray;
}
 ...
 * @date 09 Aug 2018
 * @brief Gets S array
 * @return const QMap<QUrl, QList<RouterEngine::StationStopProfile *> > SArray
 * @package RouterEngine
 * @public
 * Gets the S array for the Connection Scan Algorithm (CSA) and returns it.
 */
QMap<QUrl, QList<RouterEngine::StationStopProfile *> > RouterEngine::Planner::SArray() const
{
    return m_SArray;
}
 ...
 * @date 09 Aug 2018
 * @brief Sets S array
 * @param const QMap<QUrl, RouterEngine::TrainProfile *> &SArray
 * @package RouterEngine
 * @private
 * Sets the S array for the Connection Scan Algorithm (CSA) to the given QMap<QUrl, RouterEngine::TrainProfile *> &SArray.
 */
void RouterEngine::Planner::setSArray(const QMap<QUrl, QList<RouterEngine::StationStopProfile *> > &SArray)
{
    m_SArray = SArray;
}
